            ...
            {plans.map((p) => {
              ...
              const benefits = [
                ...
                quotaDisplay.showPeriodAmount
                  ? {
                      label: `${t('Period Quota')}: ${formatQuota(quotaDisplay.periodAmount)}`,
                      tooltip: `${t('Raw Quota')}: ${quotaDisplay.periodAmount}`,
                    }
                  : null,
                quotaDisplay.showPeriodAmount
                  ? {
                      label: `${t('Cycle Count')}: ${quotaDisplay.cycleCount}`,
                    }
                  : null,
                totalAmount > 0
                  ? {
                      label: `${t('Total Quota')}: ${formatQuota(totalAmount)}`,
                      tooltip: quotaDisplay.showPeriodAmount
                        ? `${t('Raw Quota')}: ${quotaDisplay.periodAmount} x ${quotaDisplay.cycleCount}`
                        : `${t('Raw Quota')}: ${totalAmount}`,
                    }
                  : { label: `${t('Total Quota')}: ${t('Unlimited')}` },
                limit > 0
